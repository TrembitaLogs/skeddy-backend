...
- If CI passes: task status → in_review, reassign task back to the creator/assigner
...
## PR Workflow
1. Create a NEW feature branch for EACH task (`git checkout -b feat/<task-id>-short-description`)
   - NEVER reuse branches from other tasks
   - Branch name MUST include the task identifier (e.g., feat/ske-42-add-notifications)
   - If you are already on a branch from a different task, switch to main first: `git checkout main && git pull`
2. Commit and push
...
6. If CI passes: task status → in_review, reassign task to whoever created/assigned it
...
- EVERY task MUST have its own NEW branch — never reuse branches from other tasks
- Branch naming: feat/<task-id>-short-description (e.g., feat/ske-42-add-notifications)
- Never commit directly to main
- After setting in_review: always reassign the task back to whoever created or assigned it to you
