...
# --- Test Strategy 1: PUT /auth/phone with valid E.164 number → 200 ---


async def test_update_phone_valid_e164_returns_200(app_client):
    """PUT /auth/phone with valid E.164 number → 200 {"ok": true}."""
...
# --- Test Strategy 2: PUT /auth/phone with another user's number → 409 ---


async def test_update_phone_taken_by_another_user_returns_409(app_client):
    """PUT /auth/phone with phone belonging to another user → 409 PHONE_ALREADY_EXISTS."""
...
# --- Test Strategy 3: PUT /auth/phone with phone_number=null → 200, phone removed ---


async def test_update_phone_null_removes_phone(app_client):
    """PUT /auth/phone with phone_number=null → 200, phone removed."""
...
# --- Test Strategy 4: PUT /auth/phone with same number already set → 200 (idempotent) ---


async def test_update_phone_same_number_is_idempotent(app_client):
    """PUT /auth/phone with the same number user already has → 200 (idempotent)."""
...
async def test_get_profile_shows_updated_phone(app_client):
    """GET /auth/me after PUT /auth/phone shows updated phone_number."""
...
async def test_update_phone_without_jwt_returns_401(app_client):
    """PUT /auth/phone without Authorization header → 401."""
    response = await app_client.put(
        PHONE_URL,
        json={"phone_number": "+12025551234"},
    )
